...
func parseAffectedVersions(input string) ([]Versions, error) {
	...
	if len(results) > 0 {
		firstComponent := results[0].Component
		for i, res := range results[1:] {
			if res.Component != firstComponent {
				return []Versions{}, fmt.Errorf("component should be the same, first component is %q and found %q on line %d", firstComponent, res.Component, i+1)
			}
		}
	}
